...
The scoring functions reject empty or whitespace-only candidate/reference summaries with `RangeError`. When a nonempty summary produces no built-in grams because it has fewer than `n` tokens for ROUGE-N or fewer than two tokens for ROUGE-S, the scorer returns `0`. The exported `nGram()` and `skipBigram()` utilities remain strict about their minimum input lengths, while custom gram generators define their own short-input behavior. The standalone `sentenceSegment` utility retains its single-sentence fallback for whitespace-only input.
