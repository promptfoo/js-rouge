...
By default, all three metrics segment the original text before case folding and per-sentence Penn Treebank tokenization. With `caseSensitive: false`, the built-in segmenter classifies boundaries without relying on letter casing, so summaries that become identical under JavaScript's `String.prototype.toLowerCase()` use the same boundaries. This does not implement locale-specific or full Unicode case folding; equivalences such as `ß` and `SS` are outside this guarantee. Custom segmenters still receive the original, unmodified text. The standalone `sentenceSegment()` utility retains its case-aware default; pass `{ caseNeutral: true }` to use the casing-independent boundary heuristics directly. ROUGE-N/S flatten the tokens, so grams can cross sentence boundaries. Explicitly passing `treeBankTokenize` behaves the same. Custom tokenizers receive each whole summary once in `n()`/`s()` and individual sentences in `l()`.
